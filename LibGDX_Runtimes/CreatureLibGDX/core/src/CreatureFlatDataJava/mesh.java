...
public final class mesh extends Table {
  ...
  public float[] ReadPoints()
  {
	  int numPts = pointsLength() / 2;
	  float[] retData = new float[numPts * 3];
	  for(int i = 0; i < numPts; i++)
	  {
		  int j = i * 3;
		  int k = i * 2;
		  retData[j] = points(k); 
		  retData[j + 1] = points(k + 1); 
		  retData[j + 2] = 0; 
	  }
  ...
  public float[] ReadUvs()
  {
	  float[] retData = new float[uvsLength()];
  ...
  public Vector<Integer> ReadIndices()
  {
	  Vector<Integer> retData = new Vector<Integer>();
	  for(int i = 0; i < indicesLength(); i++)
	  {
		  retData.add(indices(i));
	  }
	  return retData;
  }
