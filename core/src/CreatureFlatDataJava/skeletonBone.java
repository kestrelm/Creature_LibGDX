...
public final class skeletonBone extends Table {
  ...
  public Matrix4 ReadRestParentMat()
  {
	  float [] rawVals = new float[restParentMatLength()];
	  for(int i = 0; i < rawVals.length; i++)
	  {
		  rawVals[i] = restParentMat(i);
	  }
	  
	  return new Matrix4(rawVals);
  }
  
  public Vector3 ReadLocalRestStarPt()
  {
	  return new Vector3(localRestStartPt(0), localRestStartPt(1), 0);
  }
  
  public Vector3 ReadLocalRestEndPt()
  {
	  return new Vector3(localRestEndPt(0), localRestEndPt(1), 0);
  }

  public Vector<Integer> ReadChildren()
  {
	  Vector<Integer> retData = new Vector<Integer>();
	  for(int i = 0; i < childrenLength(); i++)
	  {
		  retData.add(children(i));
	  }
