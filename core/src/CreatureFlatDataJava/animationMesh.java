...
public final class animationMesh extends Table {
  ...
  public Vector<Vector2> ReadLocalDisplacements()
  {
	  Vector<Vector2> retList = new Vector<Vector2>();
	  for(int i = 0; i < localDisplacementsLength(); i+=2)
	  {
		  retList.add(new Vector2(localDisplacements(i), localDisplacements(i + 1)));
	  }
	  
	  return retList;
  }
  
  public Vector<Vector2> ReadPostDisplacements()
  {
	  Vector<Vector2> retList = new Vector<Vector2>();
	  for(int i = 0; i < postDisplacementsLength(); i+=2)
	  {
		  retList.add(new Vector2(postDisplacements(i), postDisplacements(i + 1)));
	  }
